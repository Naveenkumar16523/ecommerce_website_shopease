...
async function updateAuthUI() {
  ...
  // Dynamic Admin Portal Access Badge
  const existingAdminBtn = document.getElementById('navbarAdminBtn');
  if (existingAdminBtn) existingAdminBtn.remove();
  
  if (user && user.is_admin) {
    const navRight = document.querySelector('.flex.items-center.gap-2.md\\:gap-4');
    if (navRight) {
      const adminLink = document.createElement('a');
      adminLink.id = 'navbarAdminBtn';
      adminLink.href = '/admin.html';
      adminLink.className = 'flex items-center justify-center w-10 h-10 rounded-full hover:bg-white/10 transition-colors group relative text-purple-600 dark:text-purple-400';
      adminLink.title = 'ShopEase Admin Control Centre';
      adminLink.innerHTML = `
        <svg class="w-5 h-5 group-hover:scale-110 transition-transform animate-[pulse_2s_infinite]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
      ...
      if (themeToggle) {
        navRight.insertBefore(adminLink, themeToggle);
      } else {
